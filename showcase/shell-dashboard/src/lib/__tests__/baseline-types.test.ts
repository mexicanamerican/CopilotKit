...
describe("FEATURE_CATEGORIES", () => {
  ...
  it("has exactly 5 categories", () => {
    expect(Object.keys(FEATURE_CATEGORIES)).toHaveLength(5);
  });

  it("contains expected category names", () => {
    const names = Object.keys(FEATURE_CATEGORIES);
    expect(names).toContain("Chat & UI");
    expect(names).toContain("Generative UI");
    expect(names).toContain("Human in the Loop");
    expect(names).toContain("Agent Capabilities");
